...
: "Download Backups"
/opt/s3sync/s3sync.rb -r ${BACKUP_BUCKET}:${BACKUP_PREFIX:=""} /backup

: "Backup Jenkins and remove old backups"
/usr/local/bin/jenkins-backup /jenkins /backup/jenkins-backup_`date +%Y%m%d%H%M%S`.tar.gz
find /backup -type f \
...
: "Sync Backups to S3"
/opt/s3sync/s3sync.rb -r --delete /backup ${BACKUP_BUCKET}:${BACKUP_PREFIX}
